...
export function setupGUI(parentContext) {
  ...
  // ── Activities card (G1 salute) ──────────────────────────────────────
  const { card: cardActivity, body: bodyActivity } = makeCard('Activities', null, null, true);
  cardActivity.id = 'rcp-activity-card';
  cardActivity.style.display = 'none';
  const actStatusEl = document.createElement('div');
  actStatusEl.id = 'rcp-act-status';
  actStatusEl.style.cssText = 'font-size:11px;color:#34d399;margin-bottom:8px';
  actStatusEl.textContent = 'Ready';
  bodyActivity.appendChild(actStatusEl);
  const saluteGlassBtn = document.createElement('button');
  saluteGlassBtn.className = 'rcp-btn primary rcp-btn-full';
  saluteGlassBtn.innerHTML = '🫡 Salute (S)';
  bodyActivity.appendChild(saluteGlassBtn);
  scroll.appendChild(cardActivity);

  if (parentContext.params.scene === 'unitree_g1.xml') {
  ...
  parentContext.updateGUICallbacks.push((model, data, params) => {
    ...
    cardActivity.style.display = params.scene === 'unitree_g1.xml' ? 'block' : 'none';
  });

  // ── Salute wiring ────────────────────────────────────────────────────
  if (parentContext.saluteActivity) {
    const doSalute = () => {
      if (parentContext.saluteActivity.isAnimating) return;
      const statusEl = document.getElementById('rcp-act-status');
      if (statusEl) statusEl.textContent = '🫡 Saluting…';
      parentContext.saluteActivity.startSalute();
      const poll = setInterval(() => {
        if (!parentContext.saluteActivity.isAnimating) {
          clearInterval(poll);
          if (statusEl) { statusEl.textContent = '✅ Done'; setTimeout(() => { statusEl.textContent = 'Ready'; }, 1200); }
        }
      }, 100);
    };
    saluteGlassBtn.addEventListener('click', doSalute);
    document.addEventListener('keydown', (e) => {
      if (e.code === 'KeyS' && !e.ctrlKey && !e.altKey && !e.shiftKey) { doSalute(); e.preventDefault(); }
    });
  }
